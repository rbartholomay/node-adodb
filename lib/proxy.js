...
exports.exec = function (method, params, done, fail){
  ...
  // ============================================================
  // running inside an asar archive!? (electron)
  if (adodb.indexOf('app.asar') != -1) {
    var os = require('os'),
        _tmpadodb = os.tmpdir() + '/_adbodb.tmp';

    if(fs.existsSync(_tmpadodb) == false) {
      fs.writeFileSync(_tmpadodb, fs.readFileSync(adodb, 'utf8'));
    }
    adodb = _tmpadodb;
  }
  // ============================================================
