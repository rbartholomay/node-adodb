  ...
  if (!recordset.BOF || !recordset.EOF) {
    ...
    while (!recordset.EOF) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        // small patch when using adbodb with some (old) odbc providers
        // if the field type is 135 than create a timestamp if possibile -> #DEFINE ADDBTIMESTAMP 135
        if (field.Type === 135) {   
          item[field.Name] = (field.Value != null) ? new Date(field.Value) : null;
        } else {
...
var ADODB = {
  execute: function (params){
    var valid = true,
      message = 'Execute SQL: ' + params.sql + ' success !',
  ...
  executeScalar: function (params){
    var records, valid = true,
      message = 'Execute Scalar SQL: ' + params.sql + ' / ' + params.scalar + ' success !',
      connection = new ActiveXObject('ADODB.Connection'),
      recordset = new ActiveXObject('ADODB.Recordset');
